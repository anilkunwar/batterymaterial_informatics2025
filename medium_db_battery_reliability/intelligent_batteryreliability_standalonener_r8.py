...
def get_index_dir(db_path):
    ...
    try:
        ...
        st.error(f"Cannot write to temporary directory {index_dir}. Check permissions.")
        return None
    return index_dir
...
def load_spacy_model():
    ...
    try:
        # Check if model exists, if not try to download (might fail in restricted envs)
        try:
                ...
                for phrase in hyphenated_phrases:
                    for match in re.finditer(rf'\b{re.escape(phrase)}\b', doc.text, re.IGNORECASE):
                        ...
                        if start_token is not None and end_token is not None:
                            retokenizer.merge(doc[start_token:end_token])
            return doc
...
if "tokenized_sentences" not in st.session_state:
    st.session_state.tokenized_sentences = None
    ...
    try:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            with torch.no_grad():
                ...
                # Handle case where batch size is 1 (squeeze removes dim)
                if len(last_hidden_state.shape) == 1:
                    last_hidden_state = last_hidden_state.reshape(1, -1)
...
def build_sentence_index(db_path):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            if not content:
                continue
            # Truncate content to max_length to prevent spaCy crash
            doc = nlp_model(content[:nlp_model.max_length])
            ...
            update_log(f"Error processing paper {row['id']}: {e}")
            continue
    ...
    if not valid_indices:
        st.error("Failed to generate any valid embeddings. Check SciBERT installation.")
        return False
...
def hybrid_search(term, index, metadata, bm25, tokenized_sentences, top_k=50, bm25_weight=0.5):
    ...
    if term_emb is not None:
        ...
        # Search all to get distances for all sentences (since we need to combine scores)
        # Note: FAISS search returns top_k, but we need scores for all to combine with BM25.
        # For efficiency, we search for len(metadata) but FAISS might limit. 
        # Better: Calculate similarity manually if dataset is small enough, or search large k.
        # Here we assume index.search can return enough.
        D, I = index.search(term_emb, len(metadata))
    ...
    if sim_scores.max() - sim_scores.min() > 0:
...
def scientific_llm_quantified_ner_retrieval(db_file: str, max_papers: int = 500):
    ...
    if st.session_state.index is None:
        ...
        tokenized_sentences = st.session_state.tokenized_sentences
    ...
    for term in terms_to_search:
        # Retrieve more than needed to allow filtering by relevance later
        top_sentences = hybrid_search(term, index, metadata, bm25, tokenized_sentences, top_k=30)
        retrieved_sentences.extend(top_sentences)
    ...
    # Prepare context window with metadata and neighbor sentences
    # Crucial Fix: Limit context to fit model window BEFORE tokenization truncation
    ...
    safety_buffer = 100 # Reserve tokens for system prompt and JSON structure
...
Now process the retrieved excerpts below and return the JSON array."""
    ...
    # Build context text intelligently
    context_text = ""
    current_tokens = estimate_tokens(system_prompt, tokenizer)
    
    # Sort sentences by relevance (hybrid search already returns sorted, but we merged multiple terms)
    # We will just iterate and add until full
    sentences_list = list(unique_sentences.values())
    
    for s in sentences_list:
        segment = f"Paper ID: {s['paper_id']}\nTitle: {s['title']}\nYear: {s['year']}\nText: {s['text']}\n\n"
        seg_tokens = estimate_tokens(segment, tokenizer)
        
        if current_tokens + seg_tokens > safe_max_context:
            break
    ...
    # Tokenize and truncate (final safety)
                        ...
                        if use_llm and TRANSFORMERS_AVAILABLE:
                            ...
                            if tokenizer and model:
                                ...
                                if not llm_df.empty:
                                    ...
                                    st.error("Failed to load LLM or no entities found. Check logs.")
                            else:
