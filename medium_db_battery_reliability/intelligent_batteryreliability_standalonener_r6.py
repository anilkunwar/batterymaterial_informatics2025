...
try:
    ...
    st.warning("FAISS not installed. Accelerated NER will use brute‑force search (slower).")
...
# New session state for accelerated NER cache
if "sentence_index_ready" not in st.session_state:
    st.session_state.sentence_index_ready = False
if "term_query_cache" not in st.session_state:
    st.session_state.term_query_cache = {}  # key: term + top_k, value: list of (sentence, metadata)
...
# ==================== SENTENCE INDEX (NEW) ====================
@st.cache_resource(show_spinner="Building sentence index...")
def build_sentence_index(db_file):
    """
    Pre‑process all papers into sentences, compute embeddings, and store
    in a FAISS index and an SQLite table.
    Returns a tuple (faiss_index, sentences_df, sentence_embeddings).
    """
    ...
    conn.close()
    if df.empty:
        st.warning("No content found to build sentence index.")
        return None, None, None

    # Process each paper into sentences
    all_sentences = []
    all_metadata = []  # paper_id, title, year, sentence_idx, start_char, end_char
    ...
    for i in range(0, len(all_sentences), batch_size):
        ...
        emb_batch = get_scibert_embedding_batch(batch)
        embeddings_list.extend([e for e in emb_batch if e is not None])
        # If any embedding failed, we need to handle alignment
    # Filter out sentences where embedding failed
    ...
    if not valid_embeddings:
        st.error("No valid embeddings computed.")
        return None, None, None
    ...
    if FAISS_AVAILABLE:
        index = faiss.IndexFlatIP(emb_array.shape[1])  # inner product (cosine after normalization)
        index.add(emb_array)
    else:
        index = None  # we'll use brute‑force later

    # Store sentences and metadata in a DataFrame
    sentences_df = pd.DataFrame(valid_metadata)
    sentences_df["sentence_text"] = valid_sentences
    sentences_df["embedding"] = valid_embeddings  # keep for cache, but may be large

    update_log(f"Sentence index built: {len(valid_sentences)} sentences, embedding dim {emb_array.shape[1]}.")
    return index, sentences_df, emb_array

# ==================== RETRIEVAL FUNCTIONS (NEW) ====================
@st.cache_data(ttl=3600, max_entries=100)
def retrieve_relevant_sentences(term, top_k=30, hybrid_alpha=0.5):
    """
    Retrieve top‑K sentences relevant to the given term.
    Uses hybrid scoring: exact term match (via SQL LIKE) + semantic similarity (FAISS).
    Returns a list of dicts: {sentence_text, paper_id, title, year, score}
    """
    if not st.session_state.sentence_index_ready:
        st.error("Sentence index not built. Please run the index builder first.")
        return []

    index, sentences_df, emb_array = st.session_state.sentence_index
    if index is None and FAISS_AVAILABLE:
        st.error("FAISS index not available.")
        return []

    # Compute term embedding
    term_emb = get_scibert_embedding(term)
    if term_emb is None:
        return []
    term_emb = term_emb.astype(np.float32).reshape(1, -1)

    # 1. Exact match retrieval (SQL LIKE)
    # Since sentences_df is a DataFrame, we can filter using string contains
    exact_mask = sentences_df["sentence_text"].str.contains(term, case=False, regex=False)
    exact_df = sentences_df[exact_mask].copy()
    exact_df["exact_score"] = 1.0  # will be combined later

    # 2. Semantic similarity retrieval (FAISS)
    if FAISS_AVAILABLE and index is not None:
        distances, indices = index.search(term_emb, top_k)  # distances are inner products (cosine)
        sem_indices = indices[0]
        sem_scores = distances[0]
        # Remove -1 indices if any
        valid = [i for i, idx in enumerate(sem_indices) if idx != -1]
        sem_indices = [sem_indices[i] for i in valid]
        sem_scores = [sem_scores[i] for i in valid]
        sem_df = sentences_df.iloc[sem_indices].copy()
        sem_df["sem_score"] = sem_scores
    else:
        # Brute‑force cosine similarity (slower)
        scores = np.dot(emb_array, term_emb.T).flatten()
        top_indices = np.argsort(scores)[-top_k:][::-1]
        sem_df = sentences_df.iloc[top_indices].copy()
        sem_df["sem_score"] = scores[top_indices]

    # Combine results
    # Use outer join on index to combine exact and semantic scores
    combined = pd.merge(exact_df, sem_df, how="outer", left_index=True, right_index=True, suffixes=("_exact", "_sem"))
    combined["exact_score"] = combined["exact_score"].fillna(0)
    combined["sem_score"] = combined["sem_score"].fillna(0)
    combined["hybrid_score"] = hybrid_alpha * combined["exact_score"] + (1 - hybrid_alpha) * combined["sem_score"]

    # Sort by hybrid score and take top K
    combined = combined.sort_values("hybrid_score", ascending=False).head(top_k)
...
# ==================== ACCELERATED NER (NEW) ====================
def accelerated_quantified_ner(db_file, selected_terms, top_k=30, context_window=1):
    """
    For each selected term, retrieve top‑K relevant sentences and use the LLM to extract quantified entities.
    Returns a DataFrame of entities.
    """
    ...
    # Ensure sentence index is built
    if not st.session_state.sentence_index_ready:
        st.info("Building sentence index for the database...")
        index, sentences_df, emb_array = build_sentence_index(db_file)
        if index is None:
            st.error("Failed to build sentence index.")
            return pd.DataFrame()
        st.session_state.sentence_index = (index, sentences_df, emb_array)
        st.session_state.sentence_index_ready = True
    else:
        index, sentences_df, emb_array = st.session_state.sentence_index
    ...
    for i, term in enumerate(selected_terms):
        update_log(f"Processing term: {term}")
        # Retrieve relevant sentences
        retrieved = retrieve_relevant_sentences(term, top_k=top_k, hybrid_alpha=0.5)
        if not retrieved:
            update_log(f"No relevant sentences found for {term}")
            continue

        # Build context windows (include neighbor sentences)
        # For simplicity, we only use the retrieved sentences themselves (they already have context)
        # but we can optionally fetch adjacent sentences from the original papers.
        # To avoid complexity, we'll just send each retrieved sentence as a separate chunk.
        # However, the LLM may need context; we can include the sentence text as is.

        # Prepare prompts for each retrieved sentence
        for item in retrieved:
            sentence_text = item["sentence_text"]
            paper_title = item["title"]
            paper_year = item["year"]
            context = f"Paper: {paper_title} ({paper_year})\nSentence: {sentence_text}"

            prompt = f"""You are a battery reliability expert. Extract quantified degradation terms from the given sentence.
Output ONLY a JSON array of objects. Each object must have fields: term (string), value (float), unit (string), mechanism (string), context (string).
Possible mechanisms: Deformation, Fatigue, Crack and Fracture, Degradation.
If no relevant quantification is found, output an empty array.
                ...
                try:
                    ...
                    for item in items:
                        if all(k in item for k in ("term", "value", "unit", "mechanism")):
                            item["paper_id"] = item.get("paper_id", "unknown")  # could be added from metadata
                            item["title"] = paper_title
                            item["year"] = paper_year
                            item["source_term"] = term  # which term triggered this extraction
                            all_entities.append(item)
                except json.JSONDecodeError:
                    pass
        progress_bar.progress((i + 1) / len(selected_terms))
...
# ==================== TERM CATEGORIZATION (optimized version from previous) ====================
...
# ==================== LLM QUANTIFIED NER (full-text version) ====================
@st.cache_data(ttl=3600)
    ...
    with tab4:
        ...
        if st.session_state.categorized_terms or st.session_state.term_counts:
            ...
            if use_llm and TRANSFORMERS_AVAILABLE:
                ...
                # New checkbox for accelerated mode
                use_accelerated = st.checkbox("⚡ Use accelerated term‑targeted extraction (faster, less memory)", value=True)
                if use_accelerated:
                    top_k_sentences = st.slider("Number of sentences to retrieve per term", 10, 100, 30, key="top_k_sentences")
                    st.info("Accelerated mode uses a pre‑computed sentence index and only retrieves the most relevant sentences for each selected term, then extracts entities with the LLM. This is much faster and memory‑efficient.")
                    ...
                    with st.spinner("Processing NER analysis..."):
                        if use_llm and TRANSFORMERS_AVAILABLE:
                            ...
                            if tokenizer and model:
                                if use_accelerated and selected_terms:
                                    # Use accelerated retrieval-based extraction
                                    st.session_state.ner_results = accelerated_quantified_ner(
                                        UNIVERSE_DB_FILE,
                                        selected_terms,
                                    ...
                                    # Use full-text extraction (original)
                                    st.session_state.ner_results = llm_quantified_ner(UNIVERSE_DB_FILE)
                                if not st.session_state.ner_results.empty:
                                    st.session_state.last_structured_insights = {
                                        "entities": st.session_state.ner_results.to_dict(orient="records"),
                                        "summary": f"Extracted {len(st.session_state.ner_results)} quantified entities."
                                    }
                            else:
...
---
**Notes**
- **Database Inspection**: View tables, schemas, and sample data.
- **Term Categorization**: Groups terms using SciBERT embeddings, with memory‑optimised chunked processing.
- **Knowledge Graph**: Visualises relationships between categories, terms, and components.
- **NER Analysis**:
  - *Heuristic*: Uses SciBERT + rule‑based unit extraction.
  - *LLM (Full‑text)*: Processes entire papers with an LLM.
  - *Accelerated LLM* (new): Builds a sentence‑level index (SciBERT) and retrieves only the most relevant sentences for each selected term, then uses the LLM for extraction. This yields 10‑50x speedup and much lower memory usage.
- **LLM Models**: Models are loaded on CPU with low‑memory settings; previous models are unloaded to prevent leaks.
- **FAISS**: Optional for fast vector search; if not installed, falls back to brute‑force.
""")
