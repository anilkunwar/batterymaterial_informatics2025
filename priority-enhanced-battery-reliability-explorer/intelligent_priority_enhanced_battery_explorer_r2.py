...
===================================================
Hybrid version combining the flexible filtering of CODE 1 with the intelligent
parsing and physics‑aware analytics of CODE 3.

- Natural language query interface (regex + optional small LLM) replaces hard‑coded gates.
- Priority scores combine frequency, centrality, semantic similarity, and physics boost.
- Analysis router supports six failure‑focused modes (centrality, communities, pathways, etc.).
- Structured numerical insights (JSON) are generated after every analysis.
- Optional LLM summarises the insights without accessing raw data.
- Full interactive visualisation with category‑based colours and priority‑based node sizes.
"""
...
def get_data_dir() -> str:
    """Find directory containing knowledge_graph_edges.csv and knowledge_graph_nodes.csv."""
...
"""
    ...
    def hybrid_parse(self, text, tokenizer=None, model=None, use_ensemble=False, ensemble_runs=3) -> Dict:
        ...
        # Simple merge: prefer regex for numbers, otherwise use LLM (you can refine)
        merged = {}
        for k in asdict(self.defaults).keys():
        ...
        merged['confidence_score'] = 0.8  # placeholder
...
def main():
    ...
    st.markdown("<p style='text-align:center;'>Hybrid version: natural language parsing + physics‑aware analytics + structured insights</p>", unsafe_allow_html=True)
    ...
    # ------------------------------------------------------------------------
    # Run analysis based on parsed type
    # ------------------------------------------------------------------------
    if G_filtered.number_of_nodes() > 0:
        ...
        structured = st.session_state.insight_generator.generate_structured_insights(
            results, analysis_type, params, graph_stats, user_query if 'user_query' in locals() else ""
        )
        st.session_state.last_structured_insights = structured

        # Display structured insights
        with st.expander("📊 Structured Numerical Insights (JSON)", expanded=False):
            st.json(structured)
        ...
        # Optional LLM inference on insights
        if st.checkbox("🤖 Ask LLM to summarise the numerical insights (inference only)", value=False):
            with st.spinner("LLM inferring on numerical data..."):
                answer = llm_infer_on_insights(structured, user_query if 'user_query' in locals() else "",
        ...
        # --------------------------------------------------------------------
        # Visualization (adapted from CODE 1 with enhancements)
        # --------------------------------------------------------------------
        # Colors by category
        cats_in_graph = list(set([G_filtered.nodes[n].get('category','Unknown') for n in G_filtered.nodes()]))
        ...
        # Edge traces (per edge for accurate widths)
        edge_traces = []
        edge_weights = [d.get('weight',1) for u,v,d in G_filtered.edges(data=True)]
        if edge_weights:
        ...
        # --------------------------------------------------------------------
        # Additional Analytics (from CODE 1)
        # --------------------------------------------------------------------
        ...
        # Community analysis (if communities exist)
        if analysis_type == "Community Detection" and isinstance(results, dict):
            st.subheader("👥 Community Analysis")
            for cid, cdata in list(results.items())[:5]:
                with st.expander(f"Community {cid} ({len(cdata['nodes'])} nodes)"):
                    st.write("**Nodes:** " + ", ".join(cdata['nodes'][:10]))
                    if len(cdata['nodes']) > 10:
                        st.write(f"*... and {len(cdata['nodes']) - 10} more*")
                    st.write("**Top categories:** " + str(dict(cdata['categories'].most_common(3))))

        # Export static visualization
        st.sidebar.subheader("📤 Export for Publication")
        if st.sidebar.button("Generate Static Visualization"):
            static_fig = create_static_visualization(G_filtered, pos, node_colors, sizes)
        ...
        st.warning("No nodes match the current filter criteria. Try adjusting the filters or query.")
