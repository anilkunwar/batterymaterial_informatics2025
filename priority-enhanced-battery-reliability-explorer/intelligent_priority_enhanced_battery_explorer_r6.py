...
def get_data_dir() -> str:
    """Determine the data directory from environment or file structure."""
    ...
    def has_data_files(dir_path: pathlib.Path) -> bool:
        return (dir_path / "knowledge_graph_nodes.csv").exists() or \
               (dir_path / "knowledge_graph_edges.csv").exists()
...
# ============================================================================
# PHYSICS EQUATIONS AND KEY TERMS
# ============================================================================
PHYSICS_EQUATIONS = {
    "diffusion_stress": {
    ...
    "nernst_equation": {
        ...
        "variables": {"E": "Cell potential (V)", "E^0": "Standard potential (V)", "R": "Gas constant",
                      "T": "Temperature (K)", "n": "Number of electrons", "F": "Faraday constant", "Q": "Reaction quotient"},
        "unit": "V"
    },
    "butler_volmer": {
        ...
        "variables": {"j": "Current density (A/m²)", "j_0": "Exchange current density",
                      "α": "Charge transfer coefficient", "η": "Overpotential (V)"},
        "unit": "A/m²"
    },
    "heat_generation": {
        "equation": r"Q_{gen} = I^2 R + I T \frac{dU}{dT}",
        "description": "Total heat generation (Joule + reversible)",
        "variables": {"Q_gen": "Heat generation rate (W)", "I": "Current (A)", "R": "Internal resistance (Ω)",
    ...
    "thermal_runaway": {
        ...
        "variables": {"T": "Temperature (°C)", "Q_gen": "Heat generation (W)", "Q_diss": "Heat dissipation (W)",
                      "m": "Mass (kg)", "C_p": "Specific heat (J/kg·K)"},
        "unit": "°C"
    },
    "calendar_aging": {
        ...
        "variables": {"Q_loss": "Capacity loss (%)", "t": "Time (months)",
                      "E_a": "Activation energy (kJ/mol)", "T": "Temperature (K)"},
        "unit": "%/year"
    },
    "cycle_aging": {
    ...
    "diffusion_coefficient": {
        ...
        "variables": {"D": "Diffusion coefficient (cm²/s)", "D_0": "Pre-exponential factor",
...
KEY_TERMS = [
    ...
    "cycling", "stress", "diffusion", "solid electrolyte interphase", "impedance", "degrades the battery capacity",
...
def get_scibert_embedding(texts):
    """Generate SciBERT embeddings for text input."""
...
def bootstrap_centrality(G, n_samples=100, metric='degree'):
    """Compute bootstrap confidence intervals for centrality measures."""
    ...
    mean_cent, std_cent, ci_lower, ci_upper = {}, {}, {}, {}
    for n in nodes:
...
def verify_physics_equation(equation_str, variables):
    """Use sympy to verify equation consistency (simplified)."""
    try:
...
def safe_normalize(values, epsilon=1e-8):
    """Safe min-max normalization with epsilon protection."""
...
def filter_graph(G, min_weight, min_freq, selected_categories, selected_types, selected_nodes, excluded_terms, min_priority_score, suppress_low_priority):
    """Filter graph based on multiple constraints."""
    Gf = nx.Graph()
    valid = set()
...
def analyze_failure_centrality(G_filtered, focus_terms=None):
    """Analyze centrality metrics for failure-related nodes."""
...
def detect_failure_communities(G_filtered):
    """Detect communities with failure keyword analysis."""
    try:
...
def analyze_ego_networks(G_filtered, central_nodes=None):
    """Analyze ego networks around central nodes."""
...
def find_failure_pathways(G_filtered, source_terms, target_terms, require_physics=False, min_physics_similarity=0.5):
    """Find failure pathways between source and target terms."""
    pathways = {}
...
def analyze_failure_correlations(G_filtered):
    """Analyze correlations between failure terms."""
...
def llm_expand_vocabulary(query: str, tokenizer, model) -> List[str]:
    """Use LLM to generate related technical terms."""
...
def llm_suggest_missing_edges(query: str, current_nodes: List[str], tokenizer, model) -> List[Tuple[str, str, float]]:
    """Ask LLM to suggest missing edges."""
    if tokenizer is None or model is None:
        return []
...
JSON:"""
    try:
        ...
        if torch.cuda.is_available():
            inputs = inputs.to('cuda')
        with torch.no_grad():
            outputs = model.generate(inputs, max_new_tokens=200, temperature=0.2, do_sample=True, pad_token_id=tokenizer.eos_token_id)
            generated = tokenizer.decode(outputs[0], skip_special_tokens=True)
            match = re.search(r'\[.*\]', generated, re.DOTALL)
            if match:
...
def reconstruct_graph_with_attention(G_orig: nx.Graph, user_query: str, params: Dict, tokenizer, model,
                                     term_embeddings_dict: Dict[str, np.ndarray], max_nodes: int = 250, use_faiss: bool = True) -> nx.Graph:
    """Build influenced graph using attention mechanism."""
...
class DegradationInsightGenerator:
    @staticmethod
    def generate_structured_insights(analysis_results, analysis_type: str, parsed_params: Dict, graph_stats: Dict,
                                     user_query: str = "", uncertainty: Dict[str, Dict] = None) -> Dict:
        ...
        if analysis_type == "Centrality Analysis" and isinstance(analysis_results, pd.DataFrame) and not analysis_results.empty:
            ...
            for _, row in top.iterrows():
                ...
                if parsed_params.get("voltage", 3.7) > 4.2 and any(t in row["node"].lower() for t in ["voltage", "potential"]):
                    operational_boost = 1.3
                composite_weight = (0.4 * row["degree"] + 0.3 * phys_match + 0.2 * operational_boost + 0.1 * row.get("betweenness", 0))
                equation = ""
            ...
            if len(terms) > 0:
                ...
                for t1, t2, val in top_corrs[:3]:
                    data["strong_correlations"].append({"term1": t1, "term2": t2, "strength": round(val, 3)})
        ...
        if user_query:
            ...
            if query_emb is not None:
                ...
                if data["ranked_mechanisms"]:
                    data["summary"]["query_focus_score"] = round(max((m.get("query_relevance", 0) for m in data["ranked_mechanisms"]), default=0.0), 3)
...
def plot_graph(graph, title, size_attr="priority_score", use_attention_hover=False):
    ...
    for u, v, d in graph.edges(data=True):
        ...
        w = d.get('weight',1)
        if ew_max > ew_min:
            width = 0.5 + 4.5 * 0.5 * (w - ew_min) / (ew_max - ew_min)
        else:
            width = 2.0 * 0.5
        edge_traces.append(go.Scatter(x=[x0, x1, None], y=[y0, y1, None], mode='lines',
                                      line=dict(width=width, color="#888"), hoverinfo='text',
    ...
    node_trace = go.Scatter(x=node_x, y=node_y, mode='markers', hoverinfo='text', hovertext=hover_texts,
    ...
    fig.update_layout(title=title, title_font_size=20, showlegend=True, hovermode='closest',
...
def main():
    ...
    # =========================================================================
    # STEP 1: Initialize global models and embeddings
    # =========================================================================
    ...
    with st.sidebar:
        ...
        c_rate = st.slider("C-rate", OPERATIONAL_CONSTRAINTS["c_rate"]["min"], OPERATIONAL_CONSTRAINTS["c_rate"]["max"],
                          OPERATIONAL_CONSTRAINTS["c_rate"]["default"], OPERATIONAL_CONSTRAINTS["c_rate"]["step"], key="c_rate")
        voltage = st.slider("Voltage (V)", OPERATIONAL_CONSTRAINTS["voltage"]["min"], OPERATIONAL_CONSTRAINTS["voltage"]["max"],
                           OPERATIONAL_CONSTRAINTS["voltage"]["default"], OPERATIONAL_CONSTRAINTS["voltage"]["step"], key="voltage")
        temperature = st.slider("Temperature (°C)", OPERATIONAL_CONSTRAINTS["temperature"]["min"], OPERATIONAL_CONSTRAINTS["temperature"]["max"],
                               OPERATIONAL_CONSTRAINTS["temperature"]["default"], OPERATIONAL_CONSTRAINTS["temperature"]["step"], key="temperature")
        soc = st.slider("SOC (%)", OPERATIONAL_CONSTRAINTS["soc"]["min"], OPERATIONAL_CONSTRAINTS["soc"]["max"],
                       OPERATIONAL_CONSTRAINTS["soc"]["default"], OPERATIONAL_CONSTRAINTS["soc"]["step"], key="soc")
        dod = st.slider("DOD (%)", OPERATIONAL_CONSTRAINTS["dod"]["min"], OPERATIONAL_CONSTRAINTS["dod"]["max"],
                       OPERATIONAL_CONSTRAINTS["dod"]["default"], OPERATIONAL_CONSTRAINTS["dod"]["step"], key="dod")
    ...
    priority_scores = calculate_priority_scores(G, nodes_df, physics_boost_weight=physics_boost,
                                                operational_params=operational, focus_terms=params.get('focus_terms'))
    for n in G.nodes():
    ...
    G_filtered = filter_graph(G, min_weight=params['min_weight'], min_freq=params['min_freq'],
                              selected_categories=params['selected_categories'], selected_types=params['selected_types'],
                              selected_nodes=params['selected_nodes'], excluded_terms=params['excluded_terms'],
                              min_priority_score=params['min_priority_score'], suppress_low_priority=params['suppress_low_priority'])
    ...
    if use_reconstruction and run_button and user_query:
        with st.spinner("🔄 Reconstructing graph with attention + LLM..."):
            G_influenced = reconstruct_graph_with_attention(G_original, user_query, params,
                                                           st.session_state.llm_tokenizer, st.session_state.llm_model,
                                                           term_embeddings_dict, max_nodes=max_recon_nodes,
    ...
    if use_reconstruction and results_orig is not None and results_inf is not None:
        ...
        with tab1:
            ...
            df_overlap = pd.DataFrame({
                ...
                "Description": ["Similarity of node sets between Original and Influenced graphs.",
        ...
        with tab2:
            ...
            with col_left:
                ...
                fig_bar_deg = px.bar(x=["Original (filtered)", "Influenced"], y=[avg_deg_filtered, avg_deg_influenced],
                                    title="Average Node Degree", color=["Original", "Influenced"],
            ...
            with col_right:
                fig_bar_cent = px.bar(x=["Spearman ρ degree centrality"], y=[metrics["centrality_correlation"]],
                                     title="Centrality Correlation (Spearman)", range_y=[-1,1],
        ...
        with tab3:
            ...
            fig_nmi = px.bar(x=["Community NMI (Louvain)"], y=[metrics["community_nmi"]],
                            title="Community Similarity (Normalized Mutual Information)", range_y=[0,1],
            ...
            if analysis_type == "Pathway Analysis":
                ...
                for p_orig in path_orig:
                    ...
                    if p_inf:
                        ...
                        path_comp.append({"Path": name, "Length (Original)": len_orig, "Length (Influenced)": len_inf, "Delta": delta})
                if path_comp:
