...
def categorize_terms(db_file, similarity_threshold=0.7, min_freq=5):
    try:
        ...
        all_seed_terms.extend(component_terms)
        ...
        component_embeddings = [seed_embeddings_dict[term] for term in component_terms if seed_embeddings_dict[term] is not None]
        ...
        # Sort and limit terms per category
        for cat in categorized_terms:
            categorized_terms[cat] = sorted(categorized_terms[cat], key=lambda x: x[1], reverse=True)[:20]  # Limit to top 20 per category
        categorized_terms["Other"] = sorted(other_terms, key=lambda x: x[1], reverse=True)[:20]
...
---
**Notes**
- **PREKNOWLEDGE Overview**: Prioritizes key terms (e.g., electrode cracking, SEI formation) in the knowledge graph, optimized for speed with limited terms and efficient co-occurrence detection.
- **Database Inspection**: View tables, schemas, and sample data from `battery_reliability.db` or `battery_reliability_universe.db`.
- **Term Categorization**: Groups terms into Deformation, Fatigue, Crack and Fracture, Degradation using batched SciBERT embeddings.
- **Knowledge Graph**: Visualizes relationships with a focus on key terms, optimized with fewer nodes, higher min_co_occurrence, and faster layout (kamada_kawai).
...
- Check `battery_reliability_analysis.log` for detailed logs.
""")
