...
===================================================
Hybrid version with query‑driven graph reconstruction and benchmarking.
- Original graph is always preserved.
- User can enable "Reconstruct with LLM attention pooling".
- The influenced graph is built using semantic similarity, vocabulary expansion,
  attention scores, and LLM‑suggested edges.
- Numerical difference metrics (Jaccard, centrality correlation, NMI, etc.) are computed.
- Side‑by‑side visualisation of both graphs.
"""
...
# ============================================================================
# GLOBAL CONFIGURATION – DATA DIRECTORY DETECTION (from CODE 3)
# ============================================================================
...
# ============================================================================
# PHYSICS EQUATIONS AND KEY TERMS (from CODE 3)
# ============================================================================
PHYSICS_EQUATIONS = {
    "diffusion_stress": {
...
# ============================================================================
# DATA CLASS FOR PARSED PARAMETERS (from CODE 3)
# ============================================================================
@dataclass
...
# ============================================================================
# SCIBERT LOADER & EMBEDDING UTILITIES (from CODE 3)
# ============================================================================
...
# ============================================================================
# PRIORITY SCORE CALCULATION (enhanced from CODE 3)
# ============================================================================
...
# ============================================================================
# FILTER GRAPH (enhanced from CODE 3)
# ============================================================================
...
# ============================================================================
# FAILURE ANALYSIS FUNCTIONS (from CODE 2/3)
# ============================================================================
...
# ============================================================================
# NEW HELPER FUNCTIONS FOR QUERY-DRIVEN GRAPH RECONSTRUCTION
# ============================================================================
...
def llm_suggest_missing_edges(query: str, current_nodes: List[str], tokenizer, model) -> List[Tuple[str, str, float]]:
    ...
    if tokenizer is None or model is None:
        return []
    # Limit node list to avoid huge prompt
...
def reconstruct_graph_with_attention(G_orig: nx.Graph,
                                     ...
                                     tokenizer,
                                     model,
                                     term_embeddings_dict: Dict[str, np.ndarray]) -> nx.Graph:
    """
    ...
    seed_terms = [t for t in seed_terms if t]  # remove empty
    ...
    # scaled dot-product with temperature τ = 0.1
    tau = 0.1
    logits = np.dot(node_embs, q_emb) / tau
    # softmax
    exp_logits = np.exp(logits - np.max(logits))
    attention_raw = exp_logits / np.sum(exp_logits)  # shape (|V|,)

    # Step 6: Multiplicative boosts
    # a) Semantic proximity to seed terms (max cosine similarity between query and each seed)
    ...
    if seed_embs:
        ...
        sim_to_seeds = np.max(np.dot(node_embs, seed_embs.T), axis=1)  # shape (|V|,)
    else:
    ...
    # Normalise again? Not necessary; we'll use for selection and re-weighting.

    # Step 7: Node selection (top 70% by attention, or a threshold)
    threshold = np.percentile(attention, 70)
    selected_idx = np.where(attention >= threshold)[0]
    selected_nodes = [node_list[i] for i in selected_idx]
    # Build mapping from node to attention value
...
def benchmark_graphs(G_orig: nx.Graph, G_inf: nx.Graph, analysis_type: str) -> Dict:
    """Compute numerical difference metrics between two graphs."""
    # Use the full G_orig? But it may be huge. We'll compare the filtered version? Actually we compare the influenced graph to the original (unfiltered) graph for a fair benchmark.
    # To avoid scale issues, we'll consider the intersection of nodes that exist in both.
    nodes_orig = set(G_orig.nodes())
    nodes_inf = set(G_inf.nodes())
    inter_nodes = nodes_orig & nodes_inf

    # Node Jaccard
    node_jaccard = len(inter_nodes) / len(nodes_orig | nodes_inf) if (nodes_orig | nodes_inf) else 0

    # Edge Jaccard (only on induced subgraph of inter_nodes? Or all edges?)
    edges_orig = set(G_orig.edges())
    edges_inf = set(G_inf.edges())
    edge_jaccard = len(edges_orig & edges_inf) / len(edges_orig | edges_inf) if (edges_orig | edges_inf) else 0

    # Node count delta
    node_delta_pct = (len(nodes_inf) - len(nodes_orig)) / len(nodes_orig) * 100 if len(nodes_orig) > 0 else 0

    # Average degree delta (on intersection)
    deg_orig = [G_orig.degree(n) for n in inter_nodes]
    deg_inf = [G_inf.degree(n) for n in inter_nodes]
    avg_deg_orig = np.mean(deg_orig) if deg_orig else 0
    avg_deg_inf = np.mean(deg_inf) if deg_inf else 0
    avg_deg_delta = avg_deg_inf - avg_deg_orig

    # Centrality correlation (degree centrality on intersection)
    cent_orig = [nx.degree_centrality(G_orig)[n] for n in inter_nodes]
    cent_inf = [nx.degree_centrality(G_inf)[n] for n in inter_nodes]
    if len(inter_nodes) > 1:
        centrality_corr, _ = spearmanr(cent_orig, cent_inf)
    else:
    ...
    if len(inter_nodes) > 0:
        try:
            part_orig = community_louvain.best_partition(G_orig.subgraph(inter_nodes))
            part_inf = community_louvain.best_partition(G_inf.subgraph(inter_nodes))
            # Align partitions to same node order
            labels_orig = [part_orig[n] for n in inter_nodes]
            labels_inf = [part_inf[n] for n in inter_nodes]
            nmi = normalized_mutual_info_score(labels_orig, labels_inf)
        except:
            nmi = 0.0

    # Pathway length delta (if analysis_type is Pathway Analysis and we have source/target)
    pathway_delta = None
    if analysis_type == "Pathway Analysis":
        # This would need source/target lists; we'll skip for simplicity.
        pass
...
# ============================================================================
# BATTERY NLP PARSER (regex + LLM, from CODE 3)
# ============================================================================
...
"""
    ...
    def hybrid_parse(self, text, tokenizer=None, model=None, use_ensemble=False, ensemble_runs=3) -> Dict:
        ...
        # Simple merge: prefer regex for numbers, otherwise use LLM
        merged = {}
        for k in asdict(self.defaults).keys():
...
# ============================================================================
# STRUCTURED INSIGHT GENERATOR (from CODE 3)
# ============================================================================
...
# ============================================================================
# INFERENCE-ONLY LLM (from CODE 3)
# ============================================================================
...
def main():
    ...
    st.markdown("<p style='text-align:center;'>Hybrid version: query‑driven graph reconstruction + benchmarking</p>", unsafe_allow_html=True)
    ...
    # Preserve original graph for benchmarking
    G_original = G.copy()

    # Precompute node embeddings for reconstruction (cached)
    @st.cache_data
    ...
    # ------------------------------------------------------------------------
    # Sidebar filters (optional manual overrides)
    # ------------------------------------------------------------------------
    with st.sidebar:
        ...
        # --- New checkbox for reconstruction ---
    ...
    if run_button and user_query:
        with st.spinner("🔍 Parsing query..."):
            ...
            st.session_state.last_params = ParsedParameters(**params)  # convert to dataclass

            # Compute relevance (optional)
    ...
    if use_reconstruction and run_button and user_query:
        ...
        # If not reconstructing, just use the filtered graph as influenced (for comparison, but we won't compare)
        G_influenced = G_filtered.copy()

    # ------------------------------------------------------------------------
    # Run analysis on both graphs (original filtered and influenced)
    # ------------------------------------------------------------------------
    ...
    def run_analysis_on_graph(graph):
        ...
        if analysis_type == "Centrality Analysis":
            ...
            return detect_failure_communities(graph)[0]  # return only communities dict
    ...
    # Generate structured insights for both
    ...
    # Display structured insights (if both exist, maybe compare)
    if use_reconstruction and results_orig is not None and results_inf is not None:
        ...
        # Benchmark metrics
        metrics = benchmark_graphs(G_original, G_influenced, analysis_type)
        st.subheader("📏 Graph Difference Benchmark")
        st.json(metrics)
    else:
        # Just show original insights
        if results_orig is not None:
    ...
    # ------------------------------------------------------------------------
    # Visualisation (side‑by‑side if reconstruction enabled)
    # ------------------------------------------------------------------------
    if G_filtered.number_of_nodes() > 0:
        # Helper to create plot for a graph
        def plot_graph(graph, title):
            ...
            scores = [graph.nodes[n].get('priority_score',0) for n in graph.nodes()]
            min_s, max_s = 15, 60
            ...
            for node in graph.nodes():
                ...
                d = graph.nodes[node]
                node_text.append(
                    f"{node}<br>Category: {d.get('category','N/A')}<br>"
                    f"Type: {d.get('type','N/A')}<br>Frequency: {d.get('frequency',0)}<br>"
                    f"Priority: {d.get('priority_score',0):.3f}"
                )
                label = node if len(node) <= max_chars else node[:max_chars]+'...'
        ...
        if use_reconstruction and G_influenced.number_of_nodes() > 0:
            col1, col2 = st.columns(2)
            with col1:
                fig_orig = plot_graph(G_filtered, "Original Filtered Graph")
                if fig_orig:
                    st.plotly_chart(fig_orig, use_container_width=True)
            with col2:
                fig_inf = plot_graph(G_influenced, "LLM‑Influenced Graph")
                if fig_inf:
                    st.plotly_chart(fig_inf, use_container_width=True)
        else:
            fig = plot_graph(G_filtered, f"Battery Degradation Graph - {analysis_type}")
            if fig:
