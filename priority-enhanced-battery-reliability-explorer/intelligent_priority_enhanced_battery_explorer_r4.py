...
===================================================
Hybrid version with query‑driven graph reconstruction, readability improvements,
...
def main():
    ...
    st.markdown("<p style='text-align:center;'>Hybrid version: query‑driven graph reconstruction + readability + benchmark analytics</p>", unsafe_allow_html=True)
    ...
    if use_reconstruction and run_button and user_query:
        ...
        G_influenced = G_filtered.copy()  # fallback (will not be used if checkbox off)
    ...
    if use_reconstruction and results_orig is not None and results_inf is not None:
        ...
        # ---- New analytics tabs ----
        st.subheader("📊 Benchmark Comparison Analytics")
        tab1, tab2, tab3 = st.tabs(["Set Overlap", "Degree & Centrality", "Community Structure"])

        with tab1:
            df_overlap = pd.DataFrame({
                "Metric": ["Node Jaccard", "Edge Jaccard", "Node count Δ (%)"],
                "Value": [metrics["node_jaccard"], metrics["edge_jaccard"], metrics["node_count_delta_pct"]]
            })
            fig_pie = px.pie(df_overlap, values="Value", names="Metric", title="Overlap & Size Change")
        ...
        with tab2:
            ...
            with col_right:
                fig_bar_cent = px.bar(
                    ...
                    title="Centrality Correlation (higher = more similar ranking)",
                    range_y=[-1,1]
                )
        ...
        with tab3:
            fig_nmi = px.bar(
                ...
                title="Community similarity (NMI) – 1 = identical partitioning",
                range_y=[0,1]
            )
            fig_nmi.add_hline(y=0.7, line_dash="dash", annotation_text="good similarity")
    ...
    def plot_graph(graph, title, size_attr="priority_score", use_attention_hover=False):
        ...
        # No permanent text labels – only on hover
