...
# -----------------------
# 2. Helper Function for Matching Terms
# -----------------------
def map_terms_to_nodes(terms, available_nodes, n_matches=1, cutoff=0.6):
    """
    Map user-entered terms to the closest matching nodes using difflib.get_close_matches.
    
    Args:
        terms (list): List of user-entered terms.
        available_nodes (list): List of available node names in the graph.
        n_matches (int): Number of close matches to return per term.
        cutoff (float): Similarity cutoff for difflib.get_close_matches (0 to 1).
    
    Returns:
        dict: Mapping of user-entered terms to closest node names.
    """
    term_mapping = {}
    for term in terms:
        if not term.strip():
            continue
        matches = difflib.get_close_matches(term.lower(), available_nodes, n=n_matches, cutoff=cutoff)
        term_mapping[term] = matches[0] if matches else term
    return term_mapping
...
try:
    ...
    input_nodes = st.sidebar.text_input(
        "Include specific nodes (comma-separated, will match closest terms)", 
        value="electrode cracking, SEI formation, capacity fade"
    ).split(',')
    input_nodes = [t.strip().lower() for t in input_nodes if t.strip()]
    
    # Map user-entered nodes to closest graph nodes
    available_nodes = sorted(G.nodes())
    node_mapping = map_terms_to_nodes(input_nodes, available_nodes, n_matches=1, cutoff=0.6)
    selected_nodes = [node_mapping.get(term, term) for term in input_nodes if node_mapping.get(term, term) in G.nodes()]
    
    # Display mapping for user feedback
    if input_nodes:
        st.sidebar.write("**Mapped Nodes:**")
        for input_term, mapped_node in node_mapping.items():
            if mapped_node in G.nodes():
                st.sidebar.write(f"- '{input_term}' → '{mapped_node}'")
            else:
                st.sidebar.write(f"- '{input_term}' → No valid node match")

    excluded_terms = st.sidebar.text_input(
        "Exclude terms (comma-separated, will match closest terms)", 
        value="battery, material"
    ).split(',')
    excluded_terms = [t.strip().lower() for t in excluded_terms if t.strip()]
    
    # Map excluded terms to closest graph nodes
    excluded_mapping = map_terms_to_nodes(excluded_terms, available_nodes, n_matches=1, cutoff=0.6)
    mapped_excluded_terms = [excluded_mapping.get(term, term) for term in excluded_terms]
    ...
    G_filtered = filter_graph(G, min_weight, min_node_freq, selected_categories, selected_types, selected_nodes, mapped_excluded_terms, min_priority_score, suppress_low_priority)
    ...
    if analysis_type == "Centrality Analysis":
        ...
        col1, col2 = st.columns(2)
        with col1:
            input_source_terms = st.text_input(
                "Source mechanisms (comma-separated, will match closest terms)",
                value="electrode cracking, micro-cracking"
            ).split(',')
            input_source_terms = [t.strip().lower() for t in input_source_terms if t.strip()]
            source_mapping = map_terms_to_nodes(input_source_terms, available_nodes, n_matches=1, cutoff=0.6)
            source_terms = [source_mapping.get(term, term) for term in input_source_terms if source_mapping.get(term, term) in G.nodes()]
            if input_source_terms:
                st.write("**Mapped Source Terms:**")
                for input_term, mapped_node in source_mapping.items():
                    if mapped_node in G.nodes():
                        st.write(f"- '{input_term}' → '{mapped_node}'")
                    else:
                        st.write(f"- '{input_term}' → No valid node match")
        
        with col2:
            input_target_terms = st.text_input(
                "Target mechanisms (comma-separated, will match closest terms)",
                value="capacity fade, SEI formation"
            ).split(',')
            input_target_terms = [t.strip().lower() for t in input_target_terms if t.strip()]
            target_mapping = map_terms_to_nodes(input_target_terms, available_nodes, n_matches=1, cutoff=0.6)
            target_terms = [target_mapping.get(term, term) for term in input_target_terms if target_mapping.get(term, term) in G.nodes()]
            if input_target_terms:
                st.write("**Mapped Target Terms:**")
                for input_term, mapped_node in target_mapping.items():
                    if mapped_node in G.nodes():
                        st.write(f"- '{input_term}' → '{mapped_node}'")
                    else:
                        st.write(f"- '{input_term}' → No valid node match")
