...
st.title("Battery Mechanics Analysis: Deformation, Fatigue, Crack/Fracture, Degradation")
st.markdown("""
...
def inspect_database(db_path):
    try:
        ...
        if cursor.fetchone():
            ...
            terms_to_search = ["plastic deformation", "volume expansion", "cyclic loading", "crack propagation", "capacity fade"]
...
def categorize_terms(db_file, similarity_threshold=0.7, min_freq=5):
    try:
        ...
        categories = {
            ...
            "Crack and Fracture": ["crack", "cracking", "crack propagation", "crack growth", "crack initiation", "microcrack", "fracture", "fracture toughness", "brittle fracture", "ductile fracture", "crack branching", "crack tip", "stress intensity factor", "fracture mechanics", "J-integral", "cleavage fracture"],
            "Degradation": ["degradation", "capacity fade", "cycle degradation", "sei growth", "electrode degradation", "electrolyte decomposition", "aging", "thermal degradation", "mechanical degradation", "capacity loss", "fading", "deterioration", "corrosion", "passivation", "side reaction"]
        }
        ...
        for i, content in enumerate(df["content"].dropna()):
            ...
            for term in all_terms:
                ...
                elif any(np.dot(term_embedding, comp_emb) / (np.linalg.norm(term_embedding) * np.linalg.norm(comp_emb)) > similarity_threshold for comp_emb in component_embeddings if np.linalg.norm(term_embedding) != 0 and np.linalg.norm(comp_emb) != 0) and term_freqs[term] >= min_freq:
    ...
    try:
        ...
        for cat, terms in categorized_terms.items():
            for term, freq, _ in terms[:top_n]:
                G.add_node(term, type="term" if cat != "Component" else "component", freq=freq, category=cat, unit=term_units.get(cat, "None"))
                G.add_edge(cat, term, weight=1.0, type="category-term")
                term_freqs[term] = freq

        # Compute co-occurrences with physics-based unit annotations
        for content in df["content"].values:
            ...
            for term1, term2 in terms_present:
                if term1 != term2:
                    unit1 = G.nodes[term1].get("unit", "None")
                    unit2 = G.nodes[term2].get("unit", "None")
                    label = f"{unit1}-{unit2}" if unit1 != "None" or unit2 != "None" else "co-occur"
                    if G.has_edge(term1, term2):
                        G[term1][term2]["weight"] += 1
                    else:
                        G.add_edge(term1, term2, weight=1, type="term-term", label=label)
        ...
        nodes_df = pd.DataFrame([(n, d["type"], d.get("category", ""), d.get("freq", 0), d.get("unit", "None")) for n, d in G.nodes(data=True)], columns=["node", "type", "category", "frequency", "unit"])
        edges_df = pd.DataFrame([(u, v, d["weight"], d["type"], d.get("label", "")) for u, v, d in G.edges(data=True)], columns=["source", "target", "weight", "type", "label"])
...
if st.session_state.db_file:
                    ...
                    if G and G.edges():
                        ...
                        for node in G.nodes:
                            if G.nodes[node]["type"] == "category":
                                ...
                                node_sizes.append(500 + 2000 * (G.nodes[node]["freq"] / max([d.get("freq", 1) for n, d in G.nodes(data=True)], default=1)))
                        ...
                        # Draw the graph without edge labels first
                        nx.draw(G, pos, with_labels=True, node_color=node_colors, node_size=node_sizes, 
                               edge_color=edge_colors, width=edge_widths, font_size=8, font_weight="bold", ax=ax)
                        
                        # Add edge labels separately
                        edge_labels = {(u, v): G[u][v].get("label", "") for u, v in G.edges()}
                        nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, font_size=6, ax=ax)
                        
                        ax.set_title(f"Knowledge Graph of Battery Mechanics Phenomena (Top {top_n} Terms per Category, Units on Edges)")
                        plt.tight_layout()
    ...
    with tab4:
        ...
        if st.session_state.categorized_terms or st.session_state.term_counts:
            ...
            default_terms = [term for term in ["plastic deformation", "cyclic loading", "crack propagation", "capacity fade"] if term in available_terms]
...
- **Term Categorization**: Groups terms into Deformation, Fatigue, Crack and Fracture, Degradation using SciBERT embeddings.
- **Knowledge Graph**: Visualizes relationships between categories, terms, and components (Electrode, Electrolyte, Phase), with physics-based units on edges.
- **NER Analysis**: Extracts entities with context-aware unit assignment (e.g., % for Deformation strain, cycles for Fatigue life, μm for Crack length).
- **Value Histograms**: Plots distributions of numerical values for Deformation (%), Fatigue (cycles), Crack and Fracture (μm), and Degradation (%).
