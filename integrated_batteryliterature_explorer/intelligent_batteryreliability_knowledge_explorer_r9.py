...
# -*- coding: utf-8 -*-
"""
INTELLIGENT BATTERY DEGRADATION KNOWLEDGE EXPLORER (ENTHUSIASTIC WITH THE NUMERICAL SCOPE OF THE DATABASE)
======================================================================================================
A fully data‑grounded tool for exploring battery degradation concepts from a
knowledge graph.  LLM is used ONLY for parsing natural language queries into
structured parameters.  All insights are generated by a template engine that
translates graph metrics and physics equations into factual, traceable
statements – no free‑form generation.

This version builds upon the original "Battery Research Knowledge Graph Explorer"
and adds:
  - Hybrid (regex + LLM) parameter extraction (CoreShellGPT pattern)
  - Physics‑augmented priority scoring (15+ equations)
  - Operational constraint sliders (C‑rate, voltage, temperature, SOC, DOD)
  - Physics boost weight and requirement toggles
  - A pure‑template insight generator that outputs only data‑bound facts

All outputs remain strictly within the numerical scope of the loaded CSV files
and the hardcoded PHYSICS_EQUATIONS dictionary.  No new mechanisms are invented.
"""
...
def main():
    ...
    if G_filtered.number_of_nodes() > 0:
        ...
        # Visualization
        # (same as original, with category coloring)
        if G_filtered.number_of_nodes() > 0:
