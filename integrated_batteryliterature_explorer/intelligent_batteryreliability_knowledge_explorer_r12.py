                ...
                try:
                    ...
                    if paths:
                        ...
                        pathways[f"{src} -> {tgt}"] = {
                            ...
                            'nodes': path,
                            'num_paths': len(paths),
                            'contains_physics': any(any(t in n.lower() for t in PHYSICS_TERMS) for n in path),
...
# ============================================================================
# TEMPLATE-BASED INSIGHT GENERATOR (NO LLM, strictly within numerical scope)
# ============================================================================
class DegradationInsightGenerator:
    @staticmethod
    def generate_insights(analysis_results, analysis_type: str, parsed_params: Dict, graph_stats: Dict) -> str:
        lines = [f"**Graph Summary** — {graph_stats['nodes']} nodes, {graph_stats['edges']} edges"]

        if analysis_type == "Centrality Analysis" and isinstance(analysis_results, pd.DataFrame) and not analysis_results.empty:
            top = analysis_results.nlargest(3, 'degree')[['node', 'degree', 'betweenness']]
            for _, row in top.iterrows():
                eq = next((eq for name, eq in PHYSICS_EQUATIONS.items() if any(k in row['node'].lower() for k in name.lower())), None)
                eq_str = f" ({eq['equation']})" if eq else ""
                lines.append(f"• {row['node']}: degree={row['degree']:.3f}, betweenness={row['betweenness']:.3f}{eq_str}")

        elif analysis_type == "Pathway Analysis" and isinstance(analysis_results, dict):
            valid = [v for v in analysis_results.values() if v.get('path')]
            lines.append(f"• Found {len(valid)} pathways")
            for name, data in list(analysis_results.items())[:5]:
                if data['path']:
                    phys = " (contains physics term)" if data.get('contains_physics') else ""
                    lines.append(f"• {name} (length {data['length']}){phys}")

        elif analysis_type == "Community Detection" and isinstance(analysis_results, dict):
            lines.append(f"• Detected {len(analysis_results)} communities")
            for cid, data in list(analysis_results.items())[:3]:
                phys_count = sum(data.get('physics_terms', Counter()).values())
                lines.append(f"• Community {cid}: {len(data['nodes'])} nodes, {phys_count} physics terms")
            ...
            if len(terms) > 0:
                ...
                if top_corrs:
                    lines.append(f"• Strongest correlation: {top_corrs[0][0]} ↔ {top_corrs[0][1]} (strength = {top_corrs[0][2]:.2f})")
                if len(top_corrs) > 1:
                    lines.append(f"• Second strongest: {top_corrs[1][0]} ↔ {top_corrs[1][1]} ({top_corrs[1][2]:.2f})")
            ...
            if periods:
                ...
                lines.append(f"• Failure concepts: {fc_first} → {fc_last} from {first} to {last}")

        # Operational constraints
        if parsed_params.get('c_rate',1.0) != 1.0:
            lines.append(f"• C‑rate constraint: {parsed_params['c_rate']}C → rate‑related nodes boosted")
        if parsed_params.get('temperature',25.0) != 25.0:
            lines.append(f"• Temperature constraint: {parsed_params['temperature']}°C → thermal terms boosted")
        if parsed_params.get('voltage',3.7) != 3.7:
            lines.append(f"• Voltage constraint: {parsed_params['voltage']}V → voltage‑related nodes boosted")

        lines.append("\n*All statements derived directly from the loaded knowledge graph and PHYSICS_EQUATIONS.*")
        return "\n".join(lines)
...
def main():
    ...
    # NEW: search-related session state
    if "search_term" not in st.session_state:
        st.session_state.search_term = ""
    if "search_results" not in st.session_state:
        st.session_state.search_results = []
    if "highlight_search" not in st.session_state:
        st.session_state.highlight_search = True

    # Sidebar for filters (will be updated by parsed params)
    with st.sidebar:
        ...
        # ====================================================================
        # NEW: Data Searchability Section
        # ====================================================================
        st.markdown("---")
        ...
        with search_col2:
            if st.button("Search", use_container_width=True):
                ...
                # Perform search on the full graph (or filtered graph? we'll do full graph)
                if search_term:
                    matches = [node for node in G.nodes() if search_term.lower() in node.lower()]
                    st.session_state.search_results = matches
                else:
                    st.session_state.search_results = []

        if st.session_state.search_results:
            st.write(f"**{len(st.session_state.search_results)}** matching nodes:")
            # Show as a scrollable list (first 20)
            for node in st.session_state.search_results[:20]:
                st.write(f"- {node}")
            if len(st.session_state.search_results) > 20:
                st.write(f"... and {len(st.session_state.search_results)-20} more")
        else:
            if st.session_state.search_term:
                st.write("No matches found.")

        st.session_state.highlight_search = st.checkbox("Highlight search results on graph", value=st.session_state.highlight_search, key="highlight_search_check")
    ...
    if run_button and user_query:
        with st.spinner("🔍 Parsing query..."):
            ...
            # Override with current sidebar values (unless they were parsed)
            # Actually, we keep parsed params for analysis type etc., but the actual sliders are already set
            # We'll just store them for display and analysis selection
    ...
    if G_filtered.number_of_nodes() > 0:
        # Determine analysis type from last params or default
        ...
        insights = st.session_state.insight_generator.generate_insights(results, at, params_for_insights, graph_stats)
        st.session_state.last_insights = insights

        # Display insights
        st.markdown("### 📊 Analysis Results")
        if st.session_state.last_insights:
            st.markdown(f"<div style='background-color:#e8f4f8; padding:1rem; border-radius:0.5rem;'>{st.session_state.last_insights.replace(chr(10),'<br>')}</div>", unsafe_allow_html=True)
        ...
        if G_filtered.number_of_nodes() > 0:
            ...
            # Prepare search highlight set (on filtered graph)
            search_matches = set()
            if st.session_state.search_term and st.session_state.highlight_search:
                for node in G_filtered.nodes():
                    if st.session_state.search_term.lower() in node.lower():
                        search_matches.add(node)
            ...
            for n in G_filtered.nodes():
                ...
                # Determine symbol: search match gets diamond (if highlighting), else star if priority highlight, else circle
                if st.session_state.highlight_search and n in search_matches:
                    node_symbols.append('diamond')
                elif highlight and d.get('priority_score',0) > threshold:
            ...
            # Display analysis results in tabular form if applicable
            if at == "Centrality Analysis" and isinstance(results, pd.DataFrame) and not results.empty:
                st.dataframe(results[['node','degree','betweenness','closeness','category']].head(20))
            elif at == "Pathway Analysis" and isinstance(results, dict):
                valid = [v for v in results.values() if v.get('path')]
                if valid:
                    for name, data in list(results.items())[:10]:
                        if data['path']:
                            st.write(f"**{name}**: {' → '.join(data['nodes'])}")
            elif at == "Community Detection" and isinstance(results, dict):
                for cid, data in list(results.items())[:5]:
                    with st.expander(f"Community {cid} ({len(data['nodes'])} nodes)"):
                        st.write("**Physics terms:**", dict(data.get('physics_terms', Counter()).most_common(5)))
                        st.write("**Sample nodes:**", ", ".join(data['nodes'][:10]))
            elif at == "Correlation Analysis" and isinstance(results, tuple) and len(results)==2:
                corr, terms = results
                if len(terms) > 0:
