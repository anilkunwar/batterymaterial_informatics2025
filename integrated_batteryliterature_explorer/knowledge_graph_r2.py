...
try:
    ...
    # Sidebar Controls with increased font sizes
    ...
    # Apply the filter_graph function
    ...
    # COLOR NODES BY CATEGORY (UPDATED)
    if G_filtered.number_of_nodes() > 0:
        ...
        node_colors = [category_color_map.get(G_filtered.nodes[node].get('category', 'Unknown'), 'lightgray') for node in G_filtered.nodes()]
        
        # Display category legend in sidebar
        st.sidebar.subheader("🎨 Category Legend")
        for category, color in category_color_map.items():
            st.sidebar.markdown(f"<span style='color: {color}; font-size: 14px;'>●</span> {category}", unsafe_allow_html=True)
    else:
    ...
    if G_filtered.number_of_nodes() > 0:
        ...
        fig.update_layout(
            title=dict(
                text='Battery Research Knowledge Graph',
                font=dict(size=24, family='Arial, bold')  # Increased title font size
            ),
            showlegend=False,
            hovermode='closest',
            margin=dict(b=20, l=5, r=5, t=80),  # Increased top margin for larger title
            annotations=[dict(
                ...
                x=0.005,
                y=-0.002,
                font=dict(size=12)  # Increased annotation font size
            )],
    ...
    if G_filtered.number_of_nodes() > 0:
        ...
        # Additional Analytics with increased font sizes
        ...
        fig_edge.update_layout(title_font_size=18, font=dict(size=14))
        st.plotly_chart(fig_edge, use_container_width=True)
        
        # Community analysis (still using Louvain for community detection)
        if G_filtered.number_of_nodes() > 0:
    ...
    if analysis_type == "Centrality Analysis":
        ...
        if not failure_df.empty:
            ...
            fig.update_layout(title_font_size=18, font=dict(size=14))
            st.plotly_chart(fig)
