...
EXPANDED EDITION (FIXED) — PRODUCTION-GRADE BATTERY RELIABILITY ASSISTANT
...
9. Complete type safety throughout (NO list/int/float mismatches) ✓
10. All original visualizations preserved and enhanced ✓

Author: Advanced Energy Informatics Platform
Version: 5.0 (Fixed Production Edition)
Last Updated: 2025
...
# ============================================================================
# SECTION 6: OPERATIONAL CONSTRAINT CONSTANTS (FIXED - ALL FLOATS FOR CONSISTENCY)
# ============================================================================
OPERATIONAL_CONSTRAINTS = {
    "c_rate": {
        "min": 0.1,      # FIXED: float instead of int
        "max": 10.0,     # FIXED: float instead of int
        "default": 1.0,
        "step": 0.1,
        "unit": "C",
    ...
    "voltage": {
        "min": 2.5,      # FIXED: float instead of int
        "max": 4.5,      # FIXED: float instead of int
        "default": 3.7,
        "step": 0.1,
        "unit": "V",
    ...
    "temperature": {
        "min": -20.0,    # FIXED: float instead of int (-20)
        "max": 80.0,     # FIXED: float instead of int (80)
        "default": 25.0,
        "step": 1.0,
        "unit": "°C",
    ...
    "soc": {
        "min": 0.0,      # FIXED: float instead of int
        "max": 100.0,    # FIXED: float instead of int
        "default": 50.0,
        "step": 5.0,
        "unit": "%",
    ...
    for node in G.nodes():
        ...
        if emb is None or not PHYSICS_TERMS_EMBEDDINGS:
            ...
            if operational_params:
                ...
                if operational_params.get('temperature', 25) > 45 or operational_params.get('temperature', 25) < 0:
...
→ {"analysis_type": "Temporal Analysis", "temperature": 45, "focus_terms": ["thermal", "temperature"]}
...
→ {"analysis_type": "Community Detection", "temperature": 0, "focus_terms": ["plating", "lithium", "temperature"]}
...
→ {"analysis_type": "Temporal Analysis", "soc": 80, "dod": 80, "focus_terms": ["SOC", "DOD", "degradation"]}
...
try:
    ...
    with st.sidebar:
        ...
        # NEW: Operational constraints (ALL FLOATS FOR TYPE CONSISTENCY)
        ...
        c_rate = st.slider(
            "C-rate",
            float(OPERATIONAL_CONSTRAINTS['c_rate']['min']),  # FIXED: explicit float
            float(OPERATIONAL_CONSTRAINTS['c_rate']['max']),  # FIXED: explicit float
            float(current_c_rate),
            float(OPERATIONAL_CONSTRAINTS['c_rate']['step']),  # FIXED: explicit float
            key="c_rate_slider",
        ...
        voltage = st.slider(
            "Voltage (V)",
            float(OPERATIONAL_CONSTRAINTS['voltage']['min']),  # FIXED: explicit float
            float(OPERATIONAL_CONSTRAINTS['voltage']['max']),  # FIXED: explicit float
            float(current_voltage),
            float(OPERATIONAL_CONSTRAINTS['voltage']['step']),  # FIXED: explicit float
            key="voltage_slider",
            help="Cell operating voltage"
        )
        
        # FIXED: Temperature slider with all float types
        ...
        temperature = st.slider(
            "Temperature (°C)",
            float(OPERATIONAL_CONSTRAINTS['temperature']['min']),  # FIXED: -20.0 (float)
            float(OPERATIONAL_CONSTRAINTS['temperature']['max']),  # FIXED: 80.0 (float)
            float(current_temp),
            float(OPERATIONAL_CONSTRAINTS['temperature']['step']),  # FIXED: 1.0 (float)
            key="temp_slider",
        ...
        soc = st.slider(
            "State of Charge (%)",
            float(OPERATIONAL_CONSTRAINTS['soc']['min']),  # FIXED: 0.0 (float)
            float(OPERATIONAL_CONSTRAINTS['soc']['max']),  # FIXED: 100.0 (float)
            float(current_soc),
            float(OPERATIONAL_CONSTRAINTS['soc']['step']),  # FIXED: 5.0 (float)
            key="soc_slider"
        )
        ...
        dod = st.slider(
            "Depth of Discharge (%)",
            float(OPERATIONAL_CONSTRAINTS['dod']['min']),  # FIXED: 0.0 (float)
            float(OPERATIONAL_CONSTRAINTS['dod']['max']),  # FIXED: 100.0 (float)
            float(current_dod),
            float(OPERATIONAL_CONSTRAINTS['dod']['step']),  # FIXED: 5.0 (float)
            key="dod_slider"
        )
